...
"""
PythonWriter: Injects evolved rules into Python-based ABM models for fitness evaluation.
Adapted from NetLogoWriter in EvolutionaryModelDiscovery by Chathika Gunaratne (2018),
which was licensed under GNU General Public License v3.

This module supports dynamic rule creation and model instantiation
in Python-based ABMs such as StationSim.

Adapted by Gayani PDP Senanayake, 2025
Changes made:
- Replaced NetLogo-specific code with Python-based dynamic module writing
- Outputs temporary .py files for ABM evaluation
- Designed for compatibility with AGP and stochastic rule injection"""
...
class PythonWriter:
    ...
    def __init__(self, model_path: str) -> None:
        ...
        factors_file_replace = re.compile(
            re.escape("factors-file="), re.IGNORECASE
        )
        return_type_replace = re.compile(
            re.escape("return-type="), re.IGNORECASE
        )
        with open(self._original_model_path, "r") as file_reader:
            for i, line in enumerate(file_reader):
                if (
                    "@emd" in line.lower()
                    and "@evolvenextline" in line.lower()
                ):
                    self._EMD_line = i + 1
                    emd_parameters = netlogo_EMD_line_to_array(line)[3:]
                    # print(emd_parameters)
                    for emd_parameter in emd_parameters:
                        # print(emd_parameter)

                        if "return-type=" in emd_parameter.lower():
                            self._EMD_return_type = f'EMD{return_type_replace.sub("", emd_parameter)}'
                            self._EMD_return_type = slugify(
                                self._EMD_return_type
                            )


                        elif "factors-file=" in emd_parameter.lower():
                            #print(emd_parameter)
                            rel_factors_file_path = Path(
                                factors_file_replace.sub("", emd_parameter)
                            )
                            # print(rel_factors_file_path)
                            self._factors_file_path = str(
                                Path.joinpath(
                                    Path(model_path).parents[0],
                                    rel_factors_file_path,
                                )
                            )
                            # print(self._factors_file_path)

            file_reader.close()
        assert (
            self._EMD_line > 0
        ), "No @EMD @EvolveNextLine annotation detected!"
    ...
    def inject_new_rule(self, new_rule: str) -> str:
        ...
        rule_injected_model_path = Path(dir, f"{model_name}_{uniq}.EMD.py")
        rule_injected_model_path.parent.absolute().mkdir(
            parents=True, exist_ok=True
        )
        if not (Path.is_file(rule_injected_model_path)):
            # Model already injected with this rule. Using cached version.
            if self._EMD_line >= 0:
                #print(self._EMD_line)
                new_rule = "                " + new_rule
