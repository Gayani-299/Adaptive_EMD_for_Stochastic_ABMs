...
def python_EMD_line_to_array(python_EMD_line):
    assert (
        "@emd" in python_EMD_line.lower()
    ), f"Not an EMD annotated line: {python_EMD_line}"
    return re.sub("[\s#]", "", python_EMD_line).split("@")
