...
''''- Adapted by Gayani PDP Senanayake, 2025,
 - Modification: Added best_individuals tracking across AGP runs,
 - for elite retention in adaptive evolutionary search.'''
...
class EvolutionaryModelDiscovery:
    ...
    def __init__(self, model_path : str,
                    ...
                    pop_total : int, start_time: float, agg_func : Callable = np.mean) -> None:
        """
        ...
        self.replications = 1
        ModelFactors, python_writer = self._parse_model_into_factors()
        self.gp = SimpleDEAPGP(self.model_init_data, ModelFactors, python_writer)
        # print(self.gp)
    ...
    def set_selection_method(self, selection_method: str):
    ...
    def _parse_model_into_factors(self) -> 'EvolutionaryModelDiscovery.ModelFactors':
        ...
        """
        python_writer = PythonWriter(self.model_init_data['model_path'])
        ...
        # by reading in annotations from .py file and generate EMD factors
        factor_generator = FactorGenerator()
        factor_generator.generate(python_writer.get_factors_file_path())
        ...
        primitive_set_generator.generate(factor_generator.get_factors(),python_writer.get_EMD_return_type())

        module_name = get_model_factors_module_name()
        ModelFactors = importlib.import_module(f'EvolutionaryModelDiscovery.{module_name}')
        return ModelFactors, python_writer
    ...
    def evolve(self, num_procs : int = -1) -> pd.DataFrame:
        ...
        for run in range(self.replications):
            ...
            self.population, self.logbook, self.factor_scores, best_five = self.gp.evolve(num_procs=num_procs, best_individuals=self.best_individuals, start_time=self.model_init_data['start_time'])
            # Store the best individuals from this run
            self.best_individuals = best_five
            self.factor_scores.drop(columns=['ModelPath'], inplace=True)
